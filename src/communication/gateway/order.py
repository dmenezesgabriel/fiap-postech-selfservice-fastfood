...
from src.common.dto.order_dto import OrderResponseDTO, CreateOrderDTO
...
class OrderGateway(OrderGatewayInterface):
    ...
    def create(self, createOrderDTO: CreateOrderDTO):
        create_order_json = createOrderDTO.model_dump_json()
        order_response_json = httpx.post('http://localhost:8081/kitchen/v1/orders/', json=json.loads(create_order_json)).json()

        return OrderResponseDTO(**order_response_json)
